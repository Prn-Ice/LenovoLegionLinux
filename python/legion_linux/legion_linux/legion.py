...
class GraphicsModeFeature(Feature):
    ...
    def _igpu_mode_supported(self) -> bool:
        return self.igpu_mode.exists() and self._is_supported(self.igpu_mode_support)
    ...
    def set(self, mode: str):
        ...
        if mode not in choices:
            raise ValueError(f'Unsupported graphics mode {mode}; choices: {", ".join(choices)}')
        ...
        previous_igpu_mode = self.igpu_mode.get() if igpu_mode_supported else None

        requested_gsync = False if mode == self.DISCRETE else (True if gsync_supported else None)
        requested_igpu_mode = (
            {
                self.HYBRID: 0,
        ...
        try:
            ...
            current = self.get()
            if current != mode:
                raise IOError(f"Graphics mode readback mismatch: requested {mode}, got {current}")
